...
# add a minimal udevadm stub so the package post-install won't fail in build
RUN cat > /sbin/udevadm <<'EOF'
#!/bin/sh
# Minimal stub for container builds: pretend reload succeeded.
# Accept common invoked args like: control --reload-rules
# Always succeed so postinst can continue.
exit 0
EOF
RUN chmod +x /sbin/udevadm
